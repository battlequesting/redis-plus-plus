...
enum class ConnectionType { TCP = 0, UNIX };
...
struct ConnectionOptions {
  ...
  // `readonly` is only used for reading from a slave node in Redis Cluster
  // mode. Client code should never manually set/get it. This member might be
  ...
  // For internal use, and might be removed in the future. DO NOT use it in
  // client code.
...
public:
  ...
  bool broken() const noexcept { return !_ctx || _ctx->err != REDIS_OK; }
  ...
  auto
  create_time() const -> std::chrono::time_point<std::chrono::steady_clock> {
    return _create_time;
  }

  auto
  last_active() const -> std::chrono::time_point<std::chrono::steady_clock> {
    return _last_active;
  }

  template <typename... Args> void send(const char *format, Args &&...args);
  ...
  const ConnectionOptions &options() const { return _opts; }
...
inline void Connection::send(const char *format, Args &&...args) {
  ...
  if (redisAppendCommand(ctx, format, std::forward<Args>(args)...) !=
      REDIS_OK) {
