...
template <typename T> struct ParseTag {};

template <typename T> inline T parse(redisReply &reply) {
  return parse(ParseTag<T>(), reply);
}

template <typename T> T parse_leniently(redisReply &reply);
...
template <typename T,
          typename std::enable_if<IsSequenceContainer<T>::value, int>::type = 0>
T parse(ParseTag<T>, redisReply &reply);

template <typename T, typename std::enable_if<IsAssociativeContainer<T>::value,
                                              int>::type = 0>
T parse(ParseTag<T>, redisReply &reply);
...
inline bool is_map(redisReply &reply) { return reply.type == REDIS_REPLY_MAP; }

inline bool is_set(redisReply &reply) { return reply.type == REDIS_REPLY_SET; }
...
template <typename Output> void to_array(redisReply &reply, Output output);
...
template <typename T> Variant<T> parse_variant(redisReply &reply) {
...
template <typename T,
          typename std::enable_if<IsSequenceContainer<T>::value, int>::type>
T parse(ParseTag<T>, redisReply &reply) {
#ifdef REDIS_PLUS_PLUS_RESP_VERSION_3
...
template <typename T,
          typename std::enable_if<IsAssociativeContainer<T>::value, int>::type>
T parse(ParseTag<T>, redisReply &reply) {
#ifdef REDIS_PLUS_PLUS_RESP_VERSION_3
  if (!is_array(reply) && !is_map(reply) && !is_set(reply)) {
#else
