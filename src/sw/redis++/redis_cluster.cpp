...
void RedisCluster::restore(const StringView &key, const StringView &val,
                           long long ttl, bool replace) {
...
long long RedisCluster::bitcount(const StringView &key, long long start,
                                 long long end) {
...
long long RedisCluster::bitop(BitOp op, const StringView &destination,
...
long long RedisCluster::bitpos(const StringView &key, long long bit,
...
std::string RedisCluster::getrange(const StringView &key, long long start,
                                   long long end) {
...
OptionalString RedisCluster::getset(const StringView &key,
...
void RedisCluster::psetex(const StringView &key, long long ttl,
                          const StringView &val) {
...
bool RedisCluster::set(const StringView &key, const StringView &val,
                       const std::chrono::milliseconds &ttl, UpdateType type) {
...
bool RedisCluster::set(const StringView &key, const StringView &val,
                       bool keepttl, UpdateType type) {
  auto reply = command(cmd::set_keepttl, key, val, keepttl, type);

  return reply::parse_set_reply(*reply);
}

void RedisCluster::setex(const StringView &key, long long ttl,
                         const StringView &val) {
...
long long RedisCluster::setrange(const StringView &key, long long offset,
                                 const StringView &val) {
...
OptionalStringPair RedisCluster::blpop(const StringView &key,
                                       long long timeout) {
...
OptionalStringPair RedisCluster::brpop(const StringView &key,
                                       long long timeout) {
...
long long RedisCluster::linsert(const StringView &key, InsertPosition position,
...
long long RedisCluster::lrem(const StringView &key, long long count,
                             const StringView &val) {
...
void RedisCluster::lset(const StringView &key, long long index,
                        const StringView &val) {
...
void RedisCluster::ltrim(const StringView &key, long long start,
                         long long stop) {
...
OptionalString RedisCluster::rpoplpush(const StringView &source,
                                       const StringView &destination) {
...
OptionalString RedisCluster::lmove(const StringView &src,
                                   const StringView &dest,
                                   ListWhence src_whence,
                                   ListWhence dest_whence) {
  auto reply = command(cmd::lmove, src, dest, src_whence, dest_whence);

  return reply::parse<OptionalString>(*reply);
}

OptionalString RedisCluster::blmove(const StringView &src,
                                    const StringView &dest,
                                    ListWhence src_whence,
                                    ListWhence dest_whence,
                                    const std::chrono::seconds &timeout) {
  auto reply =
      command(cmd::blmove, src, dest, src_whence, dest_whence, timeout.count());
...
OptionalString RedisCluster::hget(const StringView &key,
...
long long RedisCluster::hincrby(const StringView &key, const StringView &field,
                                long long increment) {
...
double RedisCluster::hincrbyfloat(const StringView &key,
                                  const StringView &field, double increment) {
...
long long RedisCluster::hset(const StringView &key, const StringView &field,
                             const StringView &val) {
...
long long RedisCluster::hset(const StringView &key,
...
bool RedisCluster::hsetnx(const StringView &key,
                          const std::pair<StringView, StringView> &item) {
  return hsetnx(key, item.first, item.second);
}

long long RedisCluster::hstrlen(const StringView &key,
                                const StringView &field) {
...
long long RedisCluster::sunionstore(const StringView &destination,
                                    const StringView &key) {
...
auto RedisCluster::bzpopmax(const StringView &key, long long timeout)
  ...
  return reply::parse<Optional<std::tuple<std::string, std::string, double>>>(
      *reply);
}
...
long long RedisCluster::zadd(const StringView &key, const StringView &member,
                             double score, UpdateType type, bool changed) {
...
double RedisCluster::zincrby(const StringView &key, double increment,
                             const StringView &member) {
...
long long RedisCluster::zinterstore(const StringView &destination,
                                    const StringView &key, double weight) {
  auto reply = command(cmd::zinterstore, destination, key, weight);

  return reply::parse<long long>(*reply);
}

Optional<std::pair<std::string, double>>
RedisCluster::zpopmax(const StringView &key) {
...
Optional<std::pair<std::string, double>>
RedisCluster::zpopmin(const StringView &key) {
...
OptionalLongLong RedisCluster::zrank(const StringView &key,
                                     const StringView &member) {
...
long long RedisCluster::zremrangebyrank(const StringView &key, long long start,
                                        long long stop) {
...
OptionalLongLong RedisCluster::zrevrank(const StringView &key,
                                        const StringView &member) {
...
OptionalDouble RedisCluster::zscore(const StringView &key,
                                    const StringView &member) {
...
long long
RedisCluster::geoadd(const StringView &key,
...
OptionalDouble RedisCluster::geodist(const StringView &key,
                                     ...
                                     const StringView &member2, GeoUnit unit) {
  auto reply = command(cmd::geodist, key, member1, member2, unit);

  return reply::parse<OptionalDouble>(*reply);
}

OptionalString RedisCluster::geohash(const StringView &key,
                                     const StringView &member) {
...
RedisCluster::geopos(const StringView &key, const StringView &member) {
...
OptionalLongLong RedisCluster::georadius(const StringView &key,
                                         ...
                                         const StringView &destination,
                                         bool store_dist, long long count) {
  auto reply = command(cmd::georadius_store, key, loc, radius, unit,
                       destination, store_dist, count);
...
OptionalLongLong RedisCluster::georadiusbymember(const StringView &key,
                                                 ...
                                                 bool store_dist,
                                                 long long count) {
  auto reply = command(cmd::georadiusbymember_store, key, member, radius, unit,
                       destination, store_dist, count);
...
long long RedisCluster::publish(const StringView &channel,
...
long long RedisCluster::spublish(const StringView &channel,
                                 const StringView &message) {
...
long long RedisCluster::xack(const StringView &key, const StringView &group,
                             const StringView &id) {
...
void RedisCluster::xgroup_create(const StringView &key, const StringView &group,
                                 const StringView &id, bool mkstream) {
  auto reply = command(cmd::xgroup_create, key, group, id, mkstream);

  reply::parse<void>(*reply);
}

void RedisCluster::xgroup_setid(const StringView &key, const StringView &group,
                                const StringView &id) {
...
long long RedisCluster::xgroup_destroy(const StringView &key,
...
long long RedisCluster::xtrim(const StringView &key,
                              const StringView &threshold, bool approx,
                              XtrimStrategy strategy) {
  auto reply =
      command(cmd::xtrim_string_threshold, key, threshold, approx, strategy);
...
long long RedisCluster::xtrim(const StringView &key,
                              ...
                              XtrimStrategy strategy, long long limit) {
  auto reply = command(cmd::xtrim_string_threshold_limit, key, threshold,
                       strategy, limit);
